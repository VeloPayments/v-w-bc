...
vccrypt_buffer_t *match_fragment(vccrypt_buffer_t *uuid, const uint8_t *value, size_t size)
{
    ...
    if (result != VCCERT_STATUS_SUCCESS)
    {
        JS_THROW("vccert_parser_find_short");
    }
    ...
    vccert_parser_find_short(
        &fragment_parser, VCCERT_FIELD_TYPE_VELO_ENCRYPTED_SHARED_SECRET_FRAGMENT, &secret_value, &secret_size
    );
...
EMSCRIPTEN_KEEPALIVE
vccrypt_buffer_t *vwblockchain_get_field(vccrypt_buffer_t *cert, uint8_t field)
{
    int result;
    ...
    const uint8_t *value;
    size_t size;
    result = vccert_parser_find_short(&parser, field, &value, &size);
    if (result != 0)
    {
        JS_THROW("vccert_parser_find_short");
    }
...
vccrypt_buffer_t *vwblockchain_decrypt_field(vccrypt_buffer_t *secret, vccrypt_buffer_t *field)
{
    ...
    vccrypt_buffer_t *buf = malloc(sizeof(vccrypt_buffer_t));
    vccrypt_buffer_init(buf, &alloc_opts, field->size - stream.options->IV_size);
    ...
    if (result != 0)
    {
        JS_THROW("vccrypt_stream_start_decryption");
    }

    result = vccrypt_stream_decrypt(&stream, in + input_offset, field->size - input_offset, out, &offset);
    if (result != 0)
    {
        JS_THROW("vccrypt_stream_decrypt");
    }
    ...
    vccrypt_buffer_t *public_key, vccrypt_buffer_t *private_key, vccrypt_buffer_t *fragment)
{
    ...
    if (result != 0)
    {
        JS_THROW("vccrypt_key_agreement_long_term_secret_create");
    }

    result = vccrypt_block_init(&block_opts, &block, &key_buffer, false);
    if (result != 0)
    {
        JS_THROW("vccrypt_block_init");
    }

    if (fragment->size != 48)
    {
        JS_THROW("Fragment too small.");
    }
    ...
    if (result != 0)
    {
        JS_THROW("vccrypt_block_decrypt: first");
    }
    result = vccrypt_block_decrypt(&block, key_data + 16, key_data + 32, out + 16);
    if (result != 0)
    {
        JS_THROW("vccrypt_block_decrypt: second");
    }
    return buf;
}
